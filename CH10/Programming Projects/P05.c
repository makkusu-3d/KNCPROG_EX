/**
 * Modify the poker.c program of section of 10.5 by allowing
...
bool ace_straight, royal_flush, straight, flush, four, three;
...
void analyze_hand(void)
{
    int flush_check, rank_check, rank_sum, royal_flush_sum = 0, test_ace_straight = 1;
    int i = 0;
    ...
    // 12 + i * 0 + i, 1 + i, 2 + i, 3 + i
    // if i = 1
    // (12 + 1) * (0 + 1) * (1 + 1) * (2 + 1) * (3 + 1)
    // 13 * 1 * 2 * 3 * 4 = 312

    for (i = 0; i < NUM_CARDS; i++) {
        test_ace_straight *= (hand[i][0] + 1);
    }
    if (test_ace_straight == 312 ) {
        ace_straight = true;
    }
...
void print_result(void)
{
    ...
    else if (ace_straight) printf("Ace straight");
